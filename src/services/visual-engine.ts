...
// Source: Mineradio public/index.html lines 5858-6328
// ============================================================
const vertexShader = `
...
// Mineradio fragment shader (lines 6332-6357)
...
// Bloom fragment shader (lines 6367-6386)
...
// ============================================================
// VisualEngine — full Mineradio visual system port
// ============================================================
export type Preset = "silk" | "tunnel" | "orbit" | "void" | "vinyl" | "wallpaper";

const PRESET_MAP: Record<Preset, number> = {
  silk: 0, tunnel: 1, orbit: 2, void: 3, vinyl: 4, wallpaper: 5,
};
...
export class VisualEngine {
  ...
  private bloomMaterial: THREE.ShaderMaterial | null = null;
  ...
  private createDotTexture() {
    ...
    this.dotTex = new THREE.CanvasTexture(canvas);
  }

  private createRippleTexture() {
    this.rippleData = new Float32Array(12 * 4); // 12 ripples × 4 floats (x, y, age, strength)
    this.rippleTex = new THREE.DataTexture(this.rippleData, 12, 1, THREE.RGBAFormat, THREE.FloatType);
    this.rippleTex.needsUpdate = true;
  }

  private createCoverTextures() {
    // ponytail: 1x1 white placeholder until real cover loaded
    const white = new Uint8Array([255, 255, 255, 255]);
    this.coverTex = new THREE.DataTexture(white.slice(), 1, 1, THREE.RGBAFormat);
    this.coverTex.needsUpdate = true;
    this.prevCoverTex = new THREE.DataTexture(white.slice(), 1, 1, THREE.RGBAFormat);
  ...
  private buildParticles() {
    ...
    // Shared uniforms matching Mineradio
    this.uniforms = {
      uTime: { value: 0 }, uBass: { value: 0 }, uMid: { value: 0 },
      uTreble: { value: 0 }, uBeat: { value: 0 }, uEnergy: { value: 0 },
      uBurstAmt: { value: 0 }, uPreset: { value: 0 },
      uIntensity: { value: 1.0 }, uDepth: { value: 1.0 },
      uPointScale: { value: 1.0 }, uSpeed: { value: 1.0 }, uTwist: { value: 0 },
      uVinylSpin: { value: 0 }, uColorBoost: { value: 1.0 },
      uScatter: { value: 0 }, uCoverRes: { value: 1.0 }, uBgFade: { value: 0 },
      uHasCover: { value: 0 }, uHasDepth: { value: 0 },
      uEdgeEnabled: { value: 0 }, uAiBoost: { value: 0 },
      uMouseActive: { value: 0 }, uPixel: { value: devicePixelRatio },
      uColorMixT: { value: 1.0 }, uLoading: { value: 0 },
      uCoverTex: { value: this.coverTex }, uPrevCoverTex: { value: this.prevCoverTex },
      uEdgeTex: { value: this.prevCoverTex }, uRippleTex: { value: this.rippleTex },
      uRippleCount: { value: 0 }, uMouseXY: { value: new THREE.Vector2(0, 0) },
      uHandXY: { value: new THREE.Vector2(0, 0) }, uHandActive: { value: 0 },
      uGestureGrip: { value: 0 }, uTintColor: { value: new THREE.Vector3(1, 1, 1) },
      uTintStrength: { value: 0 }, uAlpha: { value: 1.0 }, uParticleDim: { value: 1.0 },
      uDotTex: { value: this.dotTex },
    };
    ...
    // Bloom layer
    this.bloomUniforms = { ...this.uniforms, uBloomStrength: { value: 0.6 } };
    this.bloomMaterial = new THREE.ShaderMaterial({
      vertexShader, fragmentShader: bloomFragmentShader, uniforms: this.bloomUniforms,
      transparent: true, depthWrite: false, blending: THREE.AdditiveBlending,
    });
    this.bloomParticles = new THREE.Points(geo, this.bloomMaterial);
    this.bloomParticles.frustumCulled = false;
    this.bloomParticles.renderOrder = -1;
  ...
  private bindEvents(container: HTMLElement) {
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      if (this.isDragging) {
        ...
        this.orbit.targetTheta -= dx;
        this.orbit.targetPhi = Math.max(-1.2, Math.min(1.2, this.orbit.targetPhi - dy));
  ...
  setCover(imageUrl: string) {
    ...
    loader.load(imageUrl, (tex) => {
      if (this.coverTex) {
        ...
        this.prevCoverTex = this.coverTex;
        this.uniforms!.uPrevCoverTex.value = this.prevCoverTex;
      }
  ...
  update(analysis: AudioAnalysis, beat: number = 0) {
    if (!this.uniforms) return;
    const u = this.uniforms;
    u.uBass.value = analysis.bass;
    u.uMid.value = analysis.mid;
    u.uTreble.value = analysis.treble;
    u.uEnergy.value = analysis.energy;
    u.uBeat.value = beat;
    u.uPreset.value = PRESET_MAP[this.preset];
    u.uMouseXY.value.set(
      this.mouse.x * 2.4,
      this.mouse.y * 2.4
    );
    u.uMouseActive.value += (this.mouseActive - u.uMouseActive.value) * 0.08;
    // Crossfade
    if (u.uColorMixT.value < 1) {
      u.uColorMixT.value = Math.min(1, u.uColorMixT.value + 0.016);
    }
    // Vinyl spin
    if (this.preset === "vinyl") {
      this.vinylSpin += 0.02;
      u.uVinylSpin.value = this.vinylSpin;
    }
  }
  ...
  private animate = () => {
    ...
    this.orbit.phi += (this.orbit.targetPhi - this.orbit.phi) * 0.10;
    this.orbit.radius += (this.orbit.targetRadius - this.orbit.radius) * 0.08;
    if (this.camera) {
      this.camera.position.x = this.orbit.radius * Math.sin(this.orbit.theta) * Math.cos(this.orbit.phi);
      this.camera.position.y = this.orbit.radius * Math.sin(this.orbit.phi);
      this.camera.position.z = this.orbit.radius * Math.cos(this.orbit.theta) * Math.cos(this.orbit.phi);
